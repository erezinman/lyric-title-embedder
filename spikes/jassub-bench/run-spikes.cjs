...
const sanity = await ev(async () => {
  const t = (label, p, ms = 15000) => Promise.race([
    Promise.resolve(p).then((v) => { console.log("OK:" + label); return v; }),
    new Promise((_, rej) => setTimeout(() => rej(new Error("TIMEOUT at " + label)), ms)),
  ]);
  try {
    const ass = await t("fetch", fetch("./sanity.ass").then((r) => r.text()));
    await t("boot", window.bench.init(ass));
    const ms = await t("render", window.bench.seekAndRender(1.0));
    const ink = await t("ink", window.bench.inkCount(0, 0, 1280, 720));
    return { firstRenderMs: +ms.toFixed(1), inkPixels: ink };
  } catch (e) { return { error: String(e) }; }
});
console.log("SANITY", JSON.stringify(sanity));
if (sanity.error || sanity.inkPixels < 100) { console.error("SANITY FAILED"); await browser.close(); process.exit(1); }
