...
class AppV2(base.App):
    ...
    def _reload_groups(self):
        if not os.path.isfile(self.json_var.get()):
            return
        try:
            self._project = make_project_v2(self.cfg())
            self._undo, self._redo = [], []
    ...
    def _rebuild_render(self):
        self._groups = project_to_render_v2(self._project) if self._project else []
        if hasattr(self, "time_scale"):
            self.time_scale.configure(to=round(base.total_duration(self._groups), 1))
        self._refresh_preview()
    ...
    # build/preview/burn reuse v1 but with v2 build
    def _generate(self):
        cfg = self.cfg()
        if not os.path.isfile(cfg["json_path"]):
            raise FileNotFoundError(cfg["json_path"])
        text, n = build_ass_v2(cfg, self._groups)
    ...
    def _render_exact(self):
        ...
        if not base.HAS_FFMPEG:
            messagebox.showinfo("No ffmpeg", "Install ffmpeg with libass for exact preview."); return
        try:
            cfg = self.cfg()
            ass_text, _ = build_ass_v2(cfg, self._groups)
            tmp_ass = os.path.join(tempfile.gettempdir(), "_ks2_preview.ass")
            open(tmp_ass, "w", encoding="utf-8").write(ass_text)
            out_png = os.path.join(tempfile.gettempdir(), "_ks2_preview.png")
            t = self.time_var.get()
            ass_f = tmp_ass.replace("\\", "\\\\").replace(":", "\\:").replace("'", "\\'")
            pw, ph = cfg["play_w"], cfg["play_h"]; dur = base.total_duration(self._groups)
            ...
            cmd = [base.FFMPEG, "-y", "-hide_banner", "-loglevel", "error", *src,
                   "-vf", f"ass='{ass_f}',scale={base.PREVIEW_W}:{self.canvas_h}", "-frames:v", "1", out_png]
            ...
            self.canvas.tag_lower("bg"); self._draw_overlay()
            self.log(f"✓ libass preview @ {base.ass_time(t)}")
        except Exception as e:
    ...
    def _apply_style_preset(self, d):
        ...
        if d.get("align") in base.ALIGN_LABELS:
            self.align_var.set(d["align"])
